...
def place_object(o: object, ix=0) -> int:
    if isinstance(o, int):
...
class ConsistentHashRing(object):
    def __init__(self, buckets, replicas=200, place_func=place_object):
        ring = []
    ...
    def target_bucket(self, o: object, ix: int=0) -> int:
        loc = self.place_func(o, ix=ix)
        return self.next_object(loc)


def uniformity(v: Iterable[int]):
...
for i in range(len(placements)):
    ...
    print("{0:.2f}% moved ({1} of {2}, {3:.2f}% theoretical)".format(
        moved[i] * 100 / total, moved[i], total, 100 / (len(placements[i]) + 1)))
